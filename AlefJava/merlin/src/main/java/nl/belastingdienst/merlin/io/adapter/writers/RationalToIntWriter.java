...
public class RationalToIntWriter extends AbstractWriter<BigRational> {
    public RationalToIntWriter(String valueTypeName) {
        super(valueTypeName);
    }

    public RationalToIntWriter(String valueTypeName, Converter<BigRational> converter) {
    ...
    public void write(ContentGenerator contentGenerator, BigRational value) throws IOException {
        if (value == null) {
            ...
            contentGenerator.writeNumber(getValueTypeName(), toOutputValue(value).toInteger());
        }
    }
}