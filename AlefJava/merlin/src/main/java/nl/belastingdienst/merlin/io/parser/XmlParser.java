...
public class XmlParser extends JacksonParser {
    ...
    @Override
    protected final ContentToken internalCurrentToken() throws IOException {
        return switch (relevantXmlState) {
            case BEGIN_COLLECTION -> ContentToken.BEGIN_COLLECTION;
            case END_COLLECTION -> ContentToken.END_COLLECTION;
            default -> super.internalCurrentToken();
        };
    }

    @Override
    protected final String internalCurrentName() throws IOException {
        return switch (relevantXmlState) {
            case BEGIN_COLLECTION, END_COLLECTION -> null;
            default -> super.internalCurrentName();
        };
    }

    @Override
    protected final String internalCurrentValue() throws IOException {
        return switch (relevantXmlState) {
            case BEGIN_COLLECTION, END_COLLECTION -> null;
            default -> super.internalCurrentValue();
        };
    }
