...
public class AdapterRegistry {
    private final Map<String, MWriter<?>> writers = new ConcurrentHashMap<>();
    private final Map<String, MReader<?>> readers = new ConcurrentHashMap<>();
    ...
    public <TAlef> void registerReader(String internalTypeName, MReader<TAlef> reader) {
        readers.put(internalTypeName, reader);
    }

    public <TAlef> MReader<TAlef> getReader(Class<TAlef> alefJavaType, String internalTypeName) {
        return getReader(alefJavaType, List.of(internalTypeName));
    }

    @SuppressWarnings("unchecked")
    public <TAlef> MReader<TAlef> getReader(Class<TAlef> alefJavaType, List<String> internalTypeNames) {
        for (String internalTypeName : internalTypeNames) {
            MReader<?> reader = readers.get(internalTypeName);
            if (reader != null) {
                return (MReader<TAlef>) reader;
            }
        }
        throw new IllegalStateException("No reader registered for any of the internal type names: " + internalTypeNames);
    }

    public <TAlef, TMessage> void registerWriter(String internalTypeName, MWriter<TAlef> writer) {
        writers.put(internalTypeName, writer);
    }

    public <TAlef> MWriter<TAlef> getWriter(Class<TAlef> alefJavaType, String internalTypeName) {
        return getWriter(alefJavaType, List.of(internalTypeName));
    }

    @SuppressWarnings("unchecked")
    public <TAlef> MWriter<TAlef> getWriter(Class<TAlef> alefJavaType, List<String> internalTypeNames) {
        for (String internalTypeName : internalTypeNames) {
            MWriter<?> writer = writers.get(internalTypeName);
            if (writer != null) {
                return (MWriter<TAlef>) writer;
            }
        }
