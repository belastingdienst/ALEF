...
public class MOutputComplexProperty<T extends MObjectType> implements MOutputField {
    ...
    private final MOutputMessage outputMessage;

    public MOutputComplexProperty(String fieldName, String collectionItemFieldName, boolean required, boolean collectionOfElements, MRoleKey roleKey, Class<T> objectTypeClass, MOutputMessage outputMessage) {
    ...
    private void generateBeginCollection(ContentGenerator generator) throws IOException {
        if (hasEnclosingElement() && !generator.isInsideKvPairSection()) {
            generator.beginEnclosedCollection(collectionItemFieldName);
        } else {
    ...
    private String getRelevantFieldName(ContentGenerator generator) {
        if (generator.isInsideKvPairSection()) {
            return collectionItemFieldName == null ? fieldName : collectionItemFieldName;
        }
        return fieldName;
    }

    private boolean hasEnclosingElement() {
        return collectionItemFieldName != null;
    }
}
