...
public abstract class AbstractOutputTest {
    protected <T> String generate(OutputField field, MPropertyKey<T> propertyKey, T value) throws IOException {
        return generate(field, propertyKey, null, value, ContentType.JSON);
    }

    protected <T> String generate(OutputField field, MPropertyKey<T> propertyKey, T value, ContentType contentType) throws IOException {
        return generate(field, propertyKey, null, value, contentType);
    }

    protected <T> String generate(OutputField field, MPropertyKey<T> propertyKey, Integer dimensionKey, T value) throws IOException {
        return generate(field, propertyKey, dimensionKey, value, ContentType.JSON);
    }

    protected <T> String generate(OutputField field, MPropertyKey<T> propertyKey, Integer dimensionKey, T value, ContentType contentType) throws IOException {
        final MUniverse universe = new MUniverse(true);
        final OutputStream outputStream = new ByteArrayOutputStream();
        final MObject alefObject = universe.getObjectType(TypeContextMock.PersonType.class).createObject();
        if (value != null) {
